...
def main(**args):
    ...
    args['batch_size'] = len(dataset_obj)
    ...
    if '01' in data_folder or '02' in data_folder or '04' in data_folder or '05' in data_folder or '10' in data_folder:
        ...
        for person_id in range(1):
            ...
            def preprocess(key_point):
                ...
                for i in range(0, total):
                    if key_point[i].shape[1] == 0:
                        key_point[i] = np.zeros([1, 118, 3])                    
                    if len( key_point[i].shape ) > 3:
                        key_point[i] = key_point[i][0]
                    if len( key_point[i].shape ) == 2:
                        key_point[i] = key_point[i][None]

                    # More than 1 person
                    if key_point[i].shape[0] > 1:
                        key_point[i] = key_point[i][:1]

                    if len( key_point[i].shape ) != 3:
                        print( key_point[i].shape )
            ...
            fit_single_frame(img, [key_1_tmp, key_2_tmp, key_3_tmp, key_4_tmp, key_5_tmp, key_6_tmp], init_trans, scan, pc,
