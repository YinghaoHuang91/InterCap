...
class Projection():
    def __init__(self, calib_dir, idx):
        if idx == 1:
            ...
            with open(osp.join(calib_dir, 'IR.json'), 'r') as f:
                self.depth_cam = json.load(f)
            with open(osp.join(calib_dir, 'Color_%d.json' % idx), 'r') as f:
                self.color_cam = json.load(f)
    ...
    def unproject_depth_image(self, depth_image, cam):
        ...
        # Rotate to different frame
        r_mat = cv2.Rodrigues(np.array(cam['R']))[0]
        xyz = np.dot(r_mat.T, (xyz_camera_space - cam['T']).T).T 
        #xyz = xyz_camera_space
    ...
    def create_scan(self, mask, depth_im, color_im=None, mask_on_color=False, coord='color', TH=1e-2, default_color=[1.00, 0.75, 0.80]):
        ...
        points = self.unproject_depth_image(depth_im, self.color_cam).reshape(-1, 3)
        ...
        #if coord == 'color':
        # YH: no transformation needed
        #if coord == 'color' and False:
        if coord == 'color':
