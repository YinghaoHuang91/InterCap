...
def fit_single_frame(img,
    ...
    '''
    try:
        fit_single_frame.counter += 1
    except:
        fit_single_frame.counter = 0
    '''
    ...
    #body_pose_prior_weights = np.array(body_pose_prior_weights) * 0.5
    #body_pose_prior_weights = body_pose_prior_weights.tolist()
    ...
    # to save scan as point clouds or not
    if True:
        m = Mesh(v=scan[0].get('points'))
        m.write_ply(body_scene_rendering_fn.replace('.png', '_depth_0.ply'))
        m = Mesh(v=scan[1].get('points'))
        m.write_ply(body_scene_rendering_fn.replace('.png', '_depth_1.ply'))
        m = Mesh(v=scan[2].get('points'))
        m.write_ply(body_scene_rendering_fn.replace('.png', '_depth_2.ply'))
        m = Mesh(v=scan[3].get('points'))
        m.write_ply(body_scene_rendering_fn.replace('.png', '_depth_3.ply'))

        m = Mesh(v=scan[4].get('points'))
        m.write_ply(body_scene_rendering_fn.replace('.png', '_depth_4.ply'))
        m = Mesh(v=scan[5].get('points'))
        m.write_ply(body_scene_rendering_fn.replace('.png', '_depth_5.ply'))
    ...
    if scan is not None:
        scan_tensor = torch.tensor(scan[0].get('points'), device=device, dtype=dtype).unsqueeze(0)
        scan_tensor_2 = torch.tensor(scan[1].get('points'), device=device, dtype=dtype).unsqueeze(0)
        scan_tensor_3 = torch.tensor(scan[2].get('points'), device=device, dtype=dtype).unsqueeze(0)
        scan_tensor_4 = torch.tensor(scan[3].get('points'), device=device, dtype=dtype).unsqueeze(0)
        scan_tensor_5 = torch.tensor(scan[4].get('points'), device=device, dtype=dtype).unsqueeze(0)
        scan_tensor_6 = torch.tensor(scan[5].get('points'), device=device, dtype=dtype).unsqueeze(0)

        scan_tensor = [scan_tensor, scan_tensor_2, scan_tensor_3, scan_tensor_4, scan_tensor_5, scan_tensor_6]
    ...
    with fitting.FittingMonitor(
        ...
        # degree rotation
        if keypoints[-1] == 0:
            shoulder_dist = torch.dist(gt_joints[0][:, left_shoulder_idx],
                                       gt_joints[0][:, right_shoulder_idx])
            ...
            cam_init_loss_val = monitor.run_fitting(camera_optimizer,
                                                    ...
                                                    pose_embedding=pose_embedding, maxiters=100,
                                                    vposer=vposer)
        ...
        for or_idx, orient in enumerate(tqdm(orientations, desc='Orientation')):
            ...
            # Only do this for the very first frame
            if keypoints[-1] == 0:
            ...
            for opt_idx, curr_weights in enumerate(tqdm(opt_weights, desc='Stage')):
                ...
                closure = monitor.create_fitting_closure(
                    ...
                    #camera=camera, gt_joints=[gt_joints, keypoints[3], keypoints[4], keypoints[5]],
                    camera=camera, gt_joints=[gt_joints, keypoints[-1]],
            ...
            if interactive:
                ...
                tqdm.write('Body final loss val = {:.5f}'.format(
                    final_loss_val if final_loss_val is not None else 0.))
    ...
    if save_meshes or visualize:
        ...
        if not os.path.exists(mesh_fn):
            ...
            out_mesh.export(mesh_fn.replace('000.ply', '000.obj'))
            #out_mesh.export(mesh_fn.replace('000_wobj.ply', '000_wobj.obj'))
