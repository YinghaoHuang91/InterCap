...
class PerspectiveCamera(nn.Module):
    ...
    def __init__(self, rotation=None, translation=None,
        ...
        if rotation is None:
            ...
            # Note here we directly take the inverse
            #rotation = np.reshape(rotation.T, [1, 3, 3])
            rotation = np.reshape(rotation, [1, 3, 3])
            rotation = torch.from_numpy(np.array(rotation, dtype=np.float32))

        rotation = nn.Parameter(rotation, requires_grad=True)
        self.register_parameter('rotation', rotation)

        translation_zero = translation

        if translation is None:
            translation = torch.zeros([batch_size, 3], dtype=dtype)
            translation_zero = torch.from_numpy(np.zeros_like(translation))
        else:
        ...
        translation = nn.Parameter(translation,
                                   requires_grad=True)
        translation_zero = nn.Parameter(translation_zero,
                                   requires_grad=True)
        self.register_parameter('translation', translation)
        self.register_parameter('translation_zero', translation_zero)
    ...
    def transform(self, points):
        ...
        camera_transform = transform_mat(self.rotation,
                                         #self.translation_zero.unsqueeze(dim=-1))
        ...
        return projected_points.squeeze()[:, :3]
